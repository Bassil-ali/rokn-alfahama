...
class MostOrderController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data = Item::whereRaw('(Select * from items inner join order_items on order_items.item_id=items.id
Group by item_id
Order by sum(item_quantity))')->latest()->paginate((request('per_page') ?? request('itemsPerPage')) ?? 15);
